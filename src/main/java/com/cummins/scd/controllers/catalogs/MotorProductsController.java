...
@RequestMapping("/_api")
//@EnableScheduling
public class MotorProductsController {

	@Autowired
	ILoadInfoService catalog;
	
	
	//filters motors-region
	@GetMapping("/list/{catalogue}/{region}")
    public ResponseEntity<?> motorXregion( @PathVariable String catalogue,@PathVariable String region){
		...
		try {
			switch(catalogue) {
			case "motorXregion":
				catalogsEntity = catalog.findMotorProductsXregion(region);
				break;
			case "mxrespm":
				catalogsEntity = catalog.findMotorRegionEspm(region);
				break;
			}
